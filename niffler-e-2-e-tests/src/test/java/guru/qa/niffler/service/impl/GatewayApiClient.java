...
public class GatewayApiClient extends RestClient {
  ...
  @Step("[Public API] send /api/friends/all GET request")
  ...
  @Step("[Public API] send /api/users/all GET request")
  ...
  @Step("[Public API] send /api/friends/remove DELETE request")
  public void removeFriend(@Nonnull String bearerToken,
                           @Nonnull String friendId) {
    final Response<Void> response;
    try {
      response = gatewayApi.removeFriend(bearerToken, friendId).execute();
    } catch (IOException e) {
      throw new AssertionError(e);
    }
    assertEquals(200, response.code());
  ...
  @Step("[Public API] send /api/friends/invite POST request")
  ...
  @Step("[Public API] send /api/friends/accept POST request")
  ...
  @Step("[Public API] send /api/friends/decline POST request")
